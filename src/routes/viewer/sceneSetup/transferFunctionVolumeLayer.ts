...
export class TransferFunctionVolumeLayer extends RemoteDataLayer {
	...
	constructor(
		...
		coarseVariableStore: VariableStore = null
	) {
	...
	async update(timestep: number) {
		...
		const coarseDataSlice = await this.coarseVariableStore.bufferStore.setCurrentSliceIndex(
			timestep,
			coarseRemoteStore
		);
		(this.renderObject.material as THREE.ShaderMaterial).uniforms.coarseVolumeTex.value =
			this.createCoarseDataTexture(coarseDataSlice);
	...
	configureUniforms(uniforms: { [uniform: string]: THREE.IUniform<any> }): void {
		...
		if (this.coarseVariableStore) {
			uniforms.coarseVolumeTex = new THREE.Uniform(
				this.createCoarseDataTexture(this.coarseVariableStore.bufferStore.allocateBuffer(1))
			);
		}
	...
	createCoarseDataTexture(dataSlice: Uint8Array): THREE.Texture {
		...
		// Create a new Uint8Array with ArrayBuffer to ensure type compatibility with THREE.js
		const data = new Uint8Array(dataSlice);
		const texture = new THREE.Data3DTexture(data, shape[1], shape[2], shape[3]);
