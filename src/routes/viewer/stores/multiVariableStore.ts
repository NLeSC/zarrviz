...
export class MultiVariableStore {
	...
	async addVariableStore(
		variable: string,
		bufferSlices: number,
		coarseningLevel: number,
		remoteStoreOrURL: RemoteStore | string
		...
		const bufferSize =
			bufferSlices *
			Math.max(1, variableInfo.numCellsXYZ[0]) *
			Math.max(1, variableInfo.numCellsXYZ[1]) *
			Math.max(1, variableInfo.numCellsXYZ[2]);
	...
	async getVariableData(
		variable: string,
		timeIndex: number,
		coarseningLevel: number = 0
	): Promise<{ data: ArrayBufferView<ArrayBufferLike>; shape: number[] }> {
		const store = this.getVariableStore(variable, coarseningLevel);
		return {
			data: await store.bufferStore.setCurrentSliceIndex(timeIndex, store.remoteStore),
			shape: store.variableInfo.numCellsXYZ
		};
	}
